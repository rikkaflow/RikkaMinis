...
object AgentTools {
    ...
    fun makeAgentTools(
        ...
        // [RAG v1] Knowledge base tools are always offered — they're
        // offline (BM25), so there's no cost to exposing them. The
        // model calls kb_list first and degrades gracefully when no
        // KB exists yet.
        ...
        if (knowledgeBaseEnabled) {
            add(kbListDefinition())
            add(kbRetrieveDefinition())
            add(kbIngestDefinition())
            add(kbCreateDefinition())
        }
    }
    ...
    // [RAG v1] Knowledge base tools (see KnowledgeBaseTools for the
    // execution side). Descriptions are the model's contract — keep them
    // aligned with KnowledgeBaseTools definitions.
    private fun kbListDefinition(): AgentToolDefinition = AgentToolDefinition(
        name = "kb_list",
    ...
    private fun kbRetrieveDefinition(): AgentToolDefinition = AgentToolDefinition(
        name = "kb_retrieve",
    ...
    private fun kbIngestDefinition(): AgentToolDefinition = AgentToolDefinition(
        name = "kb_ingest",
    ...
    private fun kbCreateDefinition(): AgentToolDefinition = AgentToolDefinition(
        name = "kb_create",
